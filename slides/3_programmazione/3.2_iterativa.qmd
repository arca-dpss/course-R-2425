...
for(i in seq_along(x)){ # per tutta la lunghezza di x = 1:10 
  somma = somma + x[i]
}
```
...
group_mean = .25 #effetto medio atteso
...
for (gruppo in gruppi) {
  ...
  for (soggetto in 1:num_soggetti) {
    ...
    RT=ifelse(gruppo == "Controllo", 
                             rnorm(1, mean = 500, sd = 50),  
              # Media diversa per il gruppo Trattamento
                             rnorm(1, mean = 450, sd = 50))
