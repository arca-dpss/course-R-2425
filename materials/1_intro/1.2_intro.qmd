...
```{r, echo = TRUE}
matrice = matrix(nrow = 2, ncol = 2, data = vettore)
matrice
```
...
::: fragment
Creo un vettore esempio in cui ci sono dentro le parole casa (ripetuta 4 volte) e bottega (ripetuta 2 volte):
```{r,echo = TRUE}
esempio = c(rep("casa",times = 4),rep("bottega", times = 2)) #?rep
esempio
```
:::
...
::: fragment
```{r,echo = TRUE}
# Quante parole dentro esempio sono uguali a casa?
esempio == "casa"
```
:::
