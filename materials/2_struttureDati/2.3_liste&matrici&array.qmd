...
```{r, echo=TRUE}
str(my_list[2]) # una parentesi []
```

***Secondo Livello***: vettore numerico

```{r, echo=TRUE}
str(my_list[[2]]) # doppia parentesi []
```

***Terzo Livello***: numero intero

```{r, echo=TRUE}
str(my_list[[2]][1]) # doppia parentesi [] e concatenazione
```
...
::: fragment
Il comando **`str()`** (struttura) è molto utile per le liste perchè fornisce una visione sulla struttura:
...
```{r, echo=TRUE}
# Simulo 30 osservazioni, assumendo che l'effetto di x = 0.3
N = 30; b1 = 0.3; sigma = 1
x = rnorm(N, 0, 1); # variabile indipedente
y = b1*x + rnorm(n = N, mean = 0, sd = sigma) # effetto più errore
```
...
my_mat2 = matrix(data = letters[1:4],nrow = 2, ncol = 2)
...
my_array = array(data = 1:6, dim = c(2,3,2)) # esempio tridimensionale
my_array
